  ...
  <div id="homePage">
    ...
    >
      <div class="search-bar">
        <a-input-search v-model:value="searchParams.searchText" placeholder="搜索">
        </a-input-search>
      </div>
    </div>
    ...
    <div v-else>
      <van-pull-refresh
        ...
        :style="activeTab === 'following' ? { marginTop: '-60px' } : {}"
        v-model="loading"
        @refresh="onRefresh"
        :distance="80"
        :head-height="60"
      >
        <div class="mobile-list-container">
          <!-- 修改关注页面空状态 -->
          <div
            v-if="activeTab === 'following' && mobileDataList.length === 0 && !loading"
            class="empty-following"
          >
            <van-empty class="custom-empty" image="search" description="暂无关注内容">
          ...
          <!-- 加载状态提示 -->
          <div
            v-if="!isEndOfData && !loading && mobileDataList.length > 0 && activeTab === 'all'"
            class="loading-more"
          >
            <van-loading type="spinner" size="24px" color="#c4947e">加载中...</van-loading>
          </div>
          <div v-if="isEndOfData && mobileDataList.length > 0" class="no-more-data-tip">
            没有更多数据了哦~
          </div>
        </div>
      </van-pull-refresh>
